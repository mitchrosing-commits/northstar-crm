...
- Password Reset MVP is available for existing local-login users with hashed reset tokens, expiry, one-time consumption, generic request responses, queued password-reset-only webhook email delivery, and dev/test-only reset link display for manual QA.
...
- Email connection status foundation is present for Gmail / Google Workspace, Microsoft 365 / Outlook, and IMAP / SMTP provider readiness in Settings. Gmail / Google Workspace can start a real OAuth connect flow when Google OAuth env and `EMAIL_TOKEN_ENCRYPTION_KEY` are configured. OAuth access and refresh tokens are stored only in encrypted `EmailConnectionSecret` rows. Connected Gmail accounts can manually sync a tiny recent metadata batch, matching only known workspace contact emails and creating deduped conservative email logs. Microsoft 365 and IMAP / SMTP remain disabled/planned.
...
- Password reset email delivery is provider-neutral, webhook-only, and queued through the `Job` table. Production reset requests enqueue `auth.password_reset_email` jobs only when an absolute reset URL can be built from `APP_BASE_URL`; `npm run jobs:work` can process queued reset email jobs continuously, and `npm run jobs:run-once` remains available for manual or scheduled one-batch processing. Repeated reset requests can enqueue multiple email jobs, but each new request consumes prior active reset tokens so stale queued links fail safely. Missing config is reported as a readiness warning, and missing or failed delivery still returns the same generic response and never displays reset links.
...
- Email provider settings show configuration readiness, a Gmail OAuth connect path, and a manual recent Gmail metadata sync action only after Gmail is connected. Gmail / Google Workspace requires OAuth env vars plus `EMAIL_TOKEN_ENCRYPTION_KEY`; Microsoft 365 / Outlook and IMAP / SMTP are still planned/disabled. Gmail sync reads recent message metadata/snippets, matches known contacts, dedupes by Gmail message id, and skips unmatched messages, full bodies, attachments, labels, deletes, sends, and whole-mailbox import.
