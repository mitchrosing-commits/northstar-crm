...
Meeting summaries are stored as concise structured sections instead of transcript-like paragraphs. Deterministic sections are limited to supported review categories: context, participants, key facts, decisions, concerns or risks, commitments, open questions, and next steps. Empty sections are omitted. Each section is marked as explicit or inferred so reviewers can distinguish stated evidence from light organization of the source material. A cleanup pass removes action owners and due-date phrases from activity titles while retaining the original evidence, keeps vague suggestions out of follow-up activities, keeps stakeholder and personal relationship details out of generic key facts, and makes missing due dates explicit in the review description.

Completed meeting activity descriptions use the structured summary, associated CRM records, and a Meeting Intelligence source-attribution line. They intentionally do not embed the full normalized markdown or transcript. The raw normalized source remains available in the review page's source preview before apply, while CRM notes and activities receive concise, human-friendly summaries with evidence snippets and target/source context.
