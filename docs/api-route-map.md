...
- Supported sources are pasted text, markdown, text files, RTF, HTML/HTM, CSV, JSON, text-based PDFs, and DOCX files. Scanned PDFs, images, audio, and video require OCR/transcription/media providers and fail with clear provider-required messages; PPTX/XLSX require export to a supported local format first; legacy `.doc` files are unsupported.
