...
- `MEETING_INTELLIGENCE_MEDIA_PROVIDER_URL`: optional provider-neutral HTTPS endpoint for Meeting Intelligence image OCR and audio/video transcription. When set, media intakes queue `meeting_intake.extract_media` jobs and the worker sends JSON with `sourceType`, `filename`, `mimeType`, and `fileBase64`. To use the first-party internal route, set this to the app's absolute `/api/internal/meeting-intelligence/media-extract` URL.
...
- `OPENAI_API_KEY`: required by the internal OpenAI media extraction route for image/whiteboard extraction and audio transcription.
