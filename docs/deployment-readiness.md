...
- `EMAIL_REPLY_OPENAI_MODEL`: optional OpenAI Responses model override for the AI Email Reply Assistant draft-generation path.
...
3. Add the OAuth consent-screen scopes `openid`, `email`, `https://www.googleapis.com/auth/gmail.readonly`, and `https://www.googleapis.com/auth/gmail.send`. Do not add `https://www.googleapis.com/auth/gmail.metadata` for Full Inbox, because Google can enforce metadata-only reads when that scope is carried with `gmail.readonly`. If the app is still in Google testing mode, add the reconnecting Gmail or Google Workspace account as a test user.
...
8. Open `/email`, connect or reconnect Gmail / Google Workspace, then run `Sync Gmail inbox` to enqueue a bounded Gmail sync job for the selected account or all connected accounts.
9. Keep a worker running (`npm run jobs:work`) or schedule one due batch (`npm run jobs:run-once`) to claim queued sync work, auto-enqueue due connected Gmail accounts, and verify inbox threads appear in `/email` under Full Inbox. Older metadata-only connections must reconnect because Full Inbox requires Gmail read and send scopes.

Gmail Full Inbox uses sign-in/profile plus Gmail read/send scopes: `openid`, `email`, `https://www.googleapis.com/auth/gmail.readonly`, and `https://www.googleapis.com/auth/gmail.send`. The same OAuth path supports consumer Gmail and Google Workspace custom-domain accounts; Google may return addresses such as `user@company.info`, `person@company.co.uk`, or subdomain-backed business addresses, and Northstar treats them as normal Gmail-backed accounts rather than requiring `gmail.com`. The OAuth URL requests those exact scopes with offline consent and disables incremental granted-scope carry-forward so older `gmail.metadata` grants are not reattached during reconnect. After callback, Northstar stores only Google-returned or Google-verified granted scopes; when the token response omits `scope`, Northstar verifies the access token with Google's tokeninfo endpoint before marking the connection Full Inbox-ready. If Google grants only sign-in/email/profile or metadata scopes, `/email` stays on reconnect-required and shows safe missing-scope categories such as Gmail read or Gmail send without exposing tokens, auth codes, raw provider responses, or headers. Sync is job-backed and workspace-scoped: the job payload contains only workspace/connection ids, token access stays encrypted in `EmailConnectionSecret`, the worker uses Gmail history cursors stored in `EmailConnection.lastSyncCursor` after the first sync, and expired or missing history falls back to a bounded recent inbox batch. Direct recent Gmail inbox sync defaults to 75 messages, while job-backed sync and load-more paths cap at 100 messages. The `/email` Sync Gmail inbox button enqueues durable work and redirects to the sync progress panel; provider calls run when `npm run jobs:work` or `npm run jobs:run-once` claims the job. The worker also enqueues due connected Gmail accounts, uses active job dedupe per connection, applies the existing retry/backoff/dead-letter path for transient failures, and skips recently attempted terminal failures to avoid infinite retry loops. If a job fails and is requeued for retry, the connected inbox keeps a subtle sync status/action strip and the detailed job/status evidence stays behind collapsed advanced diagnostics with safe job/connection references and the redacted last issue. Completed syncs update safe provider metrics on `EmailConnection`, including attempted time, successful time, imported count, duplicate count, skipped/unloaded message counts, fetched count, and sync mode; `/email` and Settings display those counts separately from current job status. Use `npm run jobs:status` to confirm due pending counts and `npm run jobs:run-once` to process one due batch with the registered Gmail handler. It stores deduped inbox messages as `EmailLog` rows with provider message/thread ids, snippets, labels, and full readable body text when available, and conservatively links known same-workspace CRM contacts when unambiguous or a sender subdomain matches a unique same-workspace organization domain. Work-priority classification does not use TLD allowlists. Gmail replies are sent only from an explicit `/email` reply form and are logged as outbound email logs. Disconnecting Gmail or Microsoft from `/email` removes the encrypted OAuth secret and marks the connection disconnected without deleting already-synced email-log snapshots. It does not auto-send AI drafts, auto-create CRM records, delete/archive provider mail, sync attachments, preserve full HTML rendering, or run Outlook/Microsoft background mailbox sync.

Operators can review Gmail Sync History and Health directly in `/email` and Settings before opening logs or Railway: each connected inbox shows the latest attempt/success, counts, manual versus automatic job source, retry or next auto-sync timing, active duplicate-job suppression, stale-worker hints, sanitized failure category, and reconnect/retry guidance. This product view uses safe refs and redacted errors only; it must not expose OAuth tokens, raw provider payloads, message bodies, stack traces, or secret-bearing job payloads.

Multi-account Gmail support is current-user scoped. A user can connect multiple Gmail or Google Workspace accounts in the same workspace, including accounts whose email differs from the Northstar login email. `/email` can sync all of that user's Gmail connections or just the selected connection, and synced `EmailLog` rows store `emailConnectionId` so duplicate provider message ids from different accounts remain separate. Replies use the source connection attached to the selected email log; disconnecting one Gmail account removes that connection's encrypted secret without deleting already-synced snapshots.

For live Gmail scope/API diagnostics, run a Railway one-off command against the web service environment such as `npm run gmail:diagnose -- --workspace northstar-revenue --actor-email alex@example.test --connection-ref 9z2ps16y --job-ref t69zgree`. The actor email may be any workspace member email domain, including Google Workspace-style custom domains. The diagnostic uses the selected connection/token path from sync and prints only sanitized categories: OAuth authorization request flags, whether the current OAuth URL requests `gmail.metadata`, consumer-Gmail versus custom-domain account classification, stored scope categories, tokeninfo scope categories and exact safe scope URLs, refresh success/failure category, Gmail list count, profile status, `messages.get` probes for `minimal`, `metadata`, `full`, and `raw` on up to two listed messages, Google HTTP status, Google error reason/status/message fields when safe, and non-secret token fingerprints so tokeninfo/list/profile/get token reuse can be confirmed. When `--job-ref` is provided, it also reports whether the Gmail sync job payload connection/workspace matches the diagnosed connection, using only short refs. It must not print access tokens, refresh tokens, auth codes, raw Gmail messages, raw headers, raw bodies, subjects, snippets, or stack traces. If tokeninfo verifies Gmail read/send but stored metadata is stale, the diagnostic repairs stored connection/secret scope metadata from tokeninfo; if full-message get still returns a permission failure, it preserves a sanitized `EMAIL_GMAIL_FULL_MESSAGE_PERMISSION_REJECTED` lastError instead of showing a clean connected state. `gmail.metadata` is reported separately and never counted as read-body access; if Google returns both `gmail.metadata` and `gmail.readonly` and Gmail rejects full/raw reads with `Metadata scope doesn't allow format ...`, the diagnostic classifies `metadata_scope_conflict` and recommends `reconnect_after_metadata_scope_removed`. If it reports missing Gmail read/send scopes, fix the Google OAuth consent screen/test user/scope setup and reconnect. If tokeninfo verifies Gmail read/send but full-message get reports `api_disabled`, enable the Gmail API for the OAuth project and reconnect/retry.

The same diagnostic can read `GMAIL_DIAGNOSTIC_WORKSPACE`, `GMAIL_DIAGNOSTIC_ACTOR_EMAIL`, `GMAIL_DIAGNOSTIC_CONNECTION_REF`, and `GMAIL_DIAGNOSTIC_JOB_REF` from the hosted environment when passing refs as command arguments is inconvenient. Failure output includes only the requested workspace/connection/job refs and a safe error code/message/status.

Live Gmail checks: confirm the Gmail API is enabled for the OAuth project, the OAuth consent screen includes `gmail.readonly` and `gmail.send`, the app publishing/testing mode allows the signed-in user, the redirect URI exactly matches `GOOGLE_OAUTH_REDIRECT_URI`, and the user reconnects Gmail after the deployed scope/API changes. If the diagnostic says Google did not grant Gmail read, fix Google Cloud consent/test-user/scope settings before retrying Northstar sync.
