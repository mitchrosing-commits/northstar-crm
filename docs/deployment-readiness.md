...
For live Gmail scope/API diagnostics, run a Railway one-off command against the web service environment such as `npm run gmail:diagnose -- --workspace northstar-revenue --actor-email alex@example.test --connection-ref 9z2ps16y --job-ref t69zgree`. The diagnostic uses the selected connection/token path from sync and prints only sanitized categories: OAuth authorization request flags, stored scope categories, tokeninfo scope categories and exact safe scope URLs, refresh success/failure category, Gmail list count, full-message get success/failure category, Google HTTP status, Google error reason, and non-secret token fingerprints so tokeninfo/list/get token reuse can be confirmed. When `--job-ref` is provided, it also reports whether the Gmail sync job payload connection/workspace matches the diagnosed connection, using only short refs. It must not print access tokens, refresh tokens, auth codes, raw Gmail messages, raw headers, raw bodies, or stack traces. If tokeninfo verifies Gmail read/send but stored metadata is stale, the diagnostic repairs stored connection/secret scope metadata from tokeninfo; if full-message get still returns a permission failure, it preserves a sanitized `EMAIL_GMAIL_FULL_MESSAGE_PERMISSION_REJECTED` lastError instead of showing a clean connected state. If it reports missing Gmail read/send scopes, fix the Google OAuth consent screen/test user/scope setup and reconnect. If tokeninfo verifies Gmail read/send but full-message get reports `api_disabled`, enable the Gmail API for the OAuth project and reconnect/retry.
