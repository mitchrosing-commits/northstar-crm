...
export function EmailDraftPanel({
  ...
  const [draftSubject, setDraftSubject] = useState(
    `Re: ${subject || initialTemplate.subject}`.slice(0, 160),
  );
  ...
  function applyTemplate(templateId: string) {
    const template =
      draftTemplates.find((item) => item.id === templateId) ??
      draftTemplates[0];
  ...
  return (
    <details
      aria-label={draftDisclosureLabel}
      className="email-draft-panel"
      title={draftDisclosureLabel}
    >
      ...
      <label className="form-field">
        ...
        <input
          readOnly
          value={recipientEmail ?? "Choose or create a contact first"}
        />
      </label>
      <label className="form-field">
