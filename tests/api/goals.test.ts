...
import {
  monthBounds,
  normalizeGoalCurrency,
  normalizeGoalTargetCents,
...
const migration = readFileSync(
  join(
    process.cwd(),
    "prisma/migrations/20260625050000_goals_v1_targets/migration.sql",
  ),
  "utf8",
);
...
const reportsActions = readFileSync(
  join(process.cwd(), "app/reports/actions.ts"),
  "utf8",
);
...
const goalsDesign = readFileSync(
  join(process.cwd(), "docs/goals-v1-design.md"),
  "utf8",
);
...
describe("Goals v1", () => {
  it("adds a workspace-scoped monthly won-revenue goal target model", () => {
    ...
    expect(migration).toContain(
      'CREATE UNIQUE INDEX "Goal_workspaceId_type_currency_periodStart_key"',
    );
  });
  ...
  it("normalizes monthly period, currency, and positive target inputs", () => {
    ...
    expect(() => monthBounds({ set: "2030-03" } as unknown as string)).toThrow(
      /valid date or YYYY-MM/,
    );
    expect(() => normalizeGoalCurrency("US")).toThrow(/three-letter/);
    expect(() =>
      normalizeGoalCurrency({ code: "USD" } as unknown as string),
    ).toThrow(/three-letter/);
    expect(() => normalizeGoalTargetCents(0)).toThrow(/positive/);
    expect(() => normalizeGoalTargetCents(goalTargetCentsMax + 1)).toThrow(
      /too large/,
    );
  });
  ...
  it("calculates progress from wonAt only and avoids UI/API surface changes", () => {
    ...
    expect(service).toContain("canManageWorkspaceSettings");
    expect(service).toContain(
      "Only workspace admins and owners can manage workspace goals.",
    );
    expect(service).toContain("goalTargetCentsMax");
    expect(service).toContain("Goal target is too large.");
    expect(service).toContain("updateMonthlyGoalIfChanged");
    expect(service).toContain("isUniqueConstraintError");
  ...
  it("adds a small table-first Reports UI using the goal service", () => {
    ...
    expect(reportsPage).toContain("canManageWorkspaceSettings(membership.role)");
    expect(reportsPage).toContain("Workspace admins and owners can save monthly goals.");
    expect(reportsPage).toContain("Members can view goal progress.");
    expect(reportsPage).toContain("Goals v1");
    expect(reportsPage).toContain(
      "Workspace-level monthly won-revenue goal only",
    );
    expect(compactSourceText(reportsPage)).toContain(
      "Progress uses same-currency WON deals whose actual won timestamp (wonAt)",
    );
    expect(reportsPage).toContain("not expected close date");
    expect(compactSourceText(reportsPage)).toContain(
      "Legacy won deals without wonAt are excluded",
    );
    ...
    expect(reportsPage).toContain(
      'aria-label="Goals v1 monthly won revenue table"',
    );
  ...
  it("validates and saves monthly goals through a focused Reports action", () => {
    expect(reportsActions).toContain(
      "export async function saveMonthlyWonRevenueGoalAction",
    );
    expect(reportsActions).toContain(
      "createOrUpdateMonthlyWonRevenueGoal(actor",
    );
    ...
    expect(reportsActions).toContain(
      "Goal target must be a positive currency amount.",
    );
    expect(reportsActions).toContain("goalTargetCentsMax");
    expect(reportsActions).toContain("Goal target is too large.");
  ...
  it("documents the Goals v1 UI MVP and remaining boundaries", () => {
    expect(goalsDesign).toContain(
      "workspace-level monthly won-revenue goal targets exist behind service functions",
    );
    ...
    expect(currentStatus).toContain("Admins and owners can save workspace goals; members can view goal progress.");
    expect(currentStatus).toContain(
      "validates targets against current integer-cent storage limits",
    );
    expect(currentStatus).toContain(
      "No charts, dashboard widgets, owner/user/team goals, quarterly goals, activity goals, or FX conversion",
    );
