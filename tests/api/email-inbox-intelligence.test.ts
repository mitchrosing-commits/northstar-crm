...
import { defaultAiPreferences, type AiPreferences } from "@/lib/services/ai-preferences-service";
...
describe("work inbox intelligence", () => {
  it("prioritizes linked customer emails that ask direct questions", () => {
    ...
    expect(inbox.items[0]).toMatchObject({
      categories: expect.arrayContaining(["priority", "work", "needs-reply", "crm-linked", "leads-opportunities"]),
      detectedIntent: "Commercial opportunity",
      priorityLevel: "high",
      relatedRecordLabel: "Deal: Expansion",
      tags: expect.arrayContaining(["Needs reply", "Pricing", "CRM linked", "AI summary"])
    });
  ...
  it("filters visible work inbox items by search, priority, CRM link, and sort", () => {
    ...
    expect(filtered.tabs.find((tab) => tab.id === "all")?.href).toContain("sort=recent");
  });
  ...
  it("honors AI preference summary length and detail level", () => {
    ...
    const detailed = buildWorkInbox({ preferences: detailedPreferences, threads: [sourceThread] }).items[0];
    const minimal = buildWorkInbox({ preferences: minimalPreferences, threads: [sourceThread] }).items[0];

    expect(detailed.summary.summary).toContain("Fourth sentence");
    expect(minimal.summary.summary).toBe("First sentence has the ask.");
    expect(detailed.reasonList.length).toBeGreaterThanOrEqual(minimal.reasonList.length);
  });

  it("normalizes unknown tabs to priority", () => {
    expect(normalizeWorkInboxTab("customers")).toBe("customers");
    expect(normalizeWorkInboxTab("unknown")).toBe("priority");
...
function thread({
  ...
  const message = {
    ...
    person: linkedRecordLabel?.startsWith("Contact:")
      ? { email: "alex@example.test", firstName: "Alex", id: "person_1", lastName: null }
      : null,
