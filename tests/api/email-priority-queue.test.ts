...
const emailPriorityQueueService = readFileSync(join(process.cwd(), "lib/services/email-priority-queue-service.ts"), "utf8");
...
describe("Relationship Inbox priority queue", () => {
  it("filters classified emails by urgent, needs-reply, and relationship-risk signals", () => {
    const emailLogs = [
      ...
      sampleEmailLog("risk_1", "Risk note", ["RELATIONSHIP_RISK", "FOLLOW_UP_NEEDED"]),
          ...
          followUps: [
            {
              ...
              id: "activity_urgent",
              linkedRecord: { href: "/deals/deal_urgent_1" as Route, label: "Deal urgent_1", type: "deal" as const },
    ...
    expect(buildEmailPriorityQueue({ emailLogs, filter: "urgent", followUpStates }).map((item) => item.emailLog.id)).toEqual([
      "urgent_1"
    ]);
    expect(buildEmailPriorityQueue({ emailLogs, filter: "needs-reply", followUpStates }).map((item) => item.emailLog.id)).toEqual([
      "urgent_1"
    ]);
    expect(buildEmailPriorityQueue({ emailLogs, filter: "relationship-risk", followUpStates }).map((item) => item.emailLog.id)).toEqual([
      "risk_1"
    ]);
    expect(buildEmailPriorityQueue({ emailLogs, followUpStates }).map((item) => item.emailLog.id)).toEqual([
      "risk_1",
      "urgent_1",
      "quote_1"
    ]);
    ...
    expect(urgentItem).toMatchObject({
      ...
      explainer: {
        ...
        evidence: expect.arrayContaining([
          ...
          expect.objectContaining({ label: "Signal: Needs reply", source: "smart_label" }),
          expect.objectContaining({ label: "Linked to deal: Deal urgent_1", source: "crm_link" }),
  ...
  it("recommends deterministic next-best actions from labels, CRM links, and follow-up state", () => {
    const openFollowUp = {
      ...
      id: "activity_open",
      linkedRecord: { href: "/deals/deal_open" as Route, label: "Open Deal", type: "deal" as const },
      source: "durable" as const,
      status: "open" as const,
    ...
    const emailLogs = [
      sampleEmailLog("unclassified_1", "Needs classification", [], "CUSTOMER", { classified: false }),
      sampleEmailLog("lead_1", "Potential lead", ["POTENTIAL_LEAD"], "PROSPECT", { linked: false }),
      sampleEmailLog("reply_1", "Customer question", ["NEEDS_REPLY"]),
      sampleEmailLog("commercial_1", "Pricing and contract", ["URGENT", "PRICING_QUOTE", "CONTRACT_LEGAL"], "CUSTOMER", {
    ...
    const byId = new Map(buildEmailPriorityQueue({ emailLogs, followUpDetails }).map((item) => [item.emailLog.id, item]));
    ...
    expect(byId.get("unclassified_1")?.explainer).toMatchObject({
      ...
      headline: "Queued because the email is unclassified but relationship-relevant.",
    ...
    expect(buildEmailPriorityQueue({ emailLogs, filter: "needs-reply", followUpDetails }).map((item) => item.emailLog.id)).toEqual([
      "reply_1"
    ]);
    ...
    expect(byId.get("lead_1")?.explainer.evidence).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ label: "Signal: Potential lead", source: "smart_label" }),
    ...
    expect(byId.get("commercial_1")?.explainer.evidence).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ label: "Signal: Urgent", source: "smart_label" }),
        expect.objectContaining({ label: "Signal: Pricing / quote", source: "smart_label" }),
        expect.objectContaining({ label: "Signal: Contract / legal", source: "smart_label" }),
        expect.objectContaining({ label: "Evidence: Evidence for Pricing and contract.", source: "smart_label" })
      ])
    );
    ...
    expect(byId.get("open_1")?.nextBestAction).toMatchObject({
      ...
      followUp: expect.objectContaining({ id: "activity_open", status: "open" }),
    ...
    expect(byId.get("completed_1")?.explainer.trail).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          followUp: expect.objectContaining({ id: "activity_completed", status: "completed" }),
    ...
    expect(byId.get("legacy_1")?.explainer.trail).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          followUp: expect.objectContaining({ id: "activity_legacy", source: "legacy" }),
          reason: expect.stringContaining("older same-record activity description marker"),
  ...
  it("builds queue summary counts and normalizes invalid filters safely", () => {
    ...
    expect(summary.find((item) => item.id === "all")).toMatchObject({ count: 3, href: "/email?inbox=all" });
    expect(summary.find((item) => item.id === "urgent")).toMatchObject({ count: 1 });
    expect(summary.find((item) => item.id === "needs-reply")).toMatchObject({ count: 1 });
    expect(summary.find((item) => item.id === "relationship-risk")).toMatchObject({ count: 1 });
    expect(summary.find((item) => item.id === "potential-leads")).toMatchObject({ count: 1 });
    expect(summary.find((item) => item.id === "prospects")).toMatchObject({ count: 1 });
  ...
  it("renders a focused queue in the Email Command Center without adding automatic mutation", () => {
    ...
    expect(emailPage).toContain("listEmailPriorityFollowUpDetails(actor, recentEmailLogs)");
    expect(emailPage).toContain("followUpDetail={followUpDetails.get(emailLog.id)}");
    ...
    expect(emailPage).toContain("Mark recommended follow-up activity ${action.followUp.title} complete");
    expect(emailPage).toContain("No Smart Label saved yet.");
    expect(emailPage).toContain("primaryLabel = action.action === \"no_action_needed\" ? \"Review email\" : action.label");
    expect(emailPage).toContain("EmailLinkedFollowUps");
    expect(emailPage).toContain("emailLinkedFollowUpStatusLabel");
    expect(emailPage).toContain('import { ActivityCompleteButton } from "@/components/activity-complete-button"');
    expect(emailPage).toContain('followUp.status === "open" ?');
    expect(emailPage).toContain("Mark linked follow-up activity ${followUp.title} complete");
    expect(emailPage).toContain("workspaceId={workspaceId}");
    expect(emailPriorityQueueService).toContain("prisma.emailLogActivityLink.findMany");
    ...
    expect(emailPriorityQueueService).toContain("Exact text offsets are not stored");
    expect(emailPriorityQueueService).toContain("Saved supporting excerpt from the Smart Label snapshot");
    expect(emailPriorityQueueService).toContain("classification.signalEvidence.find");
    expect(emailPriorityQueueService).toContain("classification.categoryEvidence");
    expect(emailPriorityQueueService).toContain("Flat saved evidence is retained for backward compatibility");
    expect(emailPriorityQueueService).toContain("Signal: ${emailSmartSignalLabel(signal)}");
    expect(emailPriorityQueueService).toContain("Legacy follow-up marker detected");
    expect(emailPriorityQueueService).toContain("Open durable linked follow-up exists");
    expect(emailPriorityQueueService).toContain("All linked follow-ups completed");
    expect(emailPriorityQueueService).toContain("emailPriorityNextBestAction");
    ...
    expect(emailPriorityQueueService).toContain("No Smart Label has been saved yet");
    expect(emailPriorityQueueService).toContain("An open linked follow-up already exists");
    expect(emailPriorityQueueService).toContain("All linked follow-ups are completed");
    expect(emailPriorityQueueService).toContain("followUps: linkedActivities");
    expect(emailPriorityQueueService).toContain("source: EmailLinkedFollowUpSource");
    expect(emailPriorityQueueService).toContain("linkedEmailLogIds");
    expect(emailPriorityQueueService).toContain("Source email: ${emailLog.subject}");
    ...
    expect(currentStatus).toContain("follow-up state uses `EmailLogActivityLink` first");
    expect(currentStatus).toContain('Deterministic next-best-action and "why this?" rules prefer durable linked follow-ups');
  });
});
...
function sampleEmailLog(
  ...
  signals: string[],
  category = "CUSTOMER",
  options: { classified?: boolean; linked?: boolean; structuredEvidence?: boolean } = {}
) {
  ...
  return {
    ...
    lead: null,
    leadId: null,
    occurredAt: new Date(`2030-01-0${id === "quote_1" ? "3" : id === "risk_1" ? "2" : "1"}T12:00:00.000Z`),
    organization: null,
    organizationId: null,
    person: linked ? { email: "buyer@example.test", firstName: "Buyer", id: `person_${id}`, lastName: "Contact" } : null,
    personId: linked ? `person_${id}` : null,
    smartLabelGeneratedAt: classified ? new Date("2030-01-01T12:00:00.000Z") : null,
