...
describe("workspace invitations MVP", () => {
  ...
  it("wires settings forms and accept links through server actions", () => {
    ...
    expect(settingsPage).toContain("Use local auth for hosted company workspaces.");
    expect(settingsPage).toContain("Demo auth and seed data are for demo-only environments");
