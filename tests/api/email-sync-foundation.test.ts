...
const migration = readFileSync(
  join(
    process.cwd(),
    "prisma/migrations/20260626010000_email_connections_foundation/migration.sql",
  ),
  "utf8",
);
...
describe("email sync provider foundation", () => {
  it("adds non-sensitive EmailConnection metadata without OAuth token storage", () => {
    ...
    expect(migration).toContain(
      "OAuth access and refresh tokens are intentionally not stored",
    );
  ...
  it("surfaces provider readiness while keeping actions disabled", () => {
    ...
    expect(emailConnectionService).toContain(
      "redactSensitiveText(value ?? undefined)",
    );
    ...
    expect(emailConnectionService).toContain(
      "normalizeGoogleOAuthScopes(tokenResponse.scope)",
    );
  ...
  it("validates OAuth env configuration without enabling plaintext token storage", () => {
    ...
    expect(
      isTokenEncryptionConfigured({
        EMAIL_TOKEN_ENCRYPTION_KEY: "x".repeat(32),
      }),
    ).toBe(true);
    expect(
      isTokenEncryptionConfigured({ EMAIL_TOKEN_ENCRYPTION_KEY: "short" }),
    ).toBe(false);
    ...
    expect(
      ...
      ok: false,
      errors: [
        "Google OAuth requires client id, client secret, and redirect URI when any provider env var is set.",
      ],
    });
    expect(
      validateRuntimeEnv({
      ...
      ok: false,
      errors: [
        "EMAIL_TOKEN_ENCRYPTION_KEY must decode to at least 32 bytes when set.",
      ],
    });
    expect(
      validateRuntimeEnv({
        ...
        MICROSOFT_OAUTH_REDIRECT_URI:
          "https://crm.example.test/api/email/microsoft/callback",
      }),
    ).toEqual({
      ok: false,
      errors: [
        "MICROSOFT_OAUTH_TENANT_ID must be a tenant id, domain, or one of: common, organizations, consumers.",
      ],
    });
    expect(
      validateRuntimeEnv({
        ...
        GOOGLE_OAUTH_REDIRECT_URI:
          "https://crm.example.test/api/email/google/callback",
      }),
    ).toMatchObject({
      ok: true,
      warnings: [
        "Email OAuth env is configured, but EMAIL_TOKEN_ENCRYPTION_KEY is not set; provider connection buttons stay disabled.",
      ],
    });
  ...
  it("keeps seed and integration cleanup explicit without fake connected providers", () => {
    ...
    expect(currentStatus).toContain(
      "OAuth access and refresh tokens are stored only in encrypted",
    );
  });
  ...
  it("adds Full Inbox provider metadata, Gmail read/send scopes, and explicit reply UI", () => {
    const fullInboxMigration = readFileSync(
      join(
        process.cwd(),
        "prisma/migrations/20260706130000_email_full_inbox_metadata/migration.sql",
      ),
      "utf8",
    );
    ...
    expect(emailConnectionService).toContain(
      "export async function enqueueGmailInboxSyncJob",
    );
    expect(emailConnectionService).toContain(
    ...
    expect(emailConnectionService).toContain(
      "markJobFailedForRetry(claimedJob.id",
    );
    ...
    expect(emailConnectionService).toContain(
      "parseGmailHistoryCursor(connection.lastSyncCursor)",
    );
    expect(emailConnectionService).toContain(
    ...
    expect(emailConnectionService).toContain(
      "syncRecentGmailInboxMessagesForConnection",
    );
    expect(emailConnectionService).toContain(
      "enqueueGmailInboxSyncJobForConnection(actor, connection.id)",
    );
    expect(emailConnectionService).toContain(
