...
const migration = readFileSync(
  join(
    process.cwd(),
    "prisma/migrations/20260626020000_email_connection_encrypted_tokens/migration.sql",
  ),
  "utf8",
);
...
const gmailDiagnoseScript = readFileSync(
  join(process.cwd(), "scripts/gmail-diagnose.ts"),
  "utf8",
);
const packageJson = readFileSync(join(process.cwd(), "package.json"), "utf8");
const connectRoute = readFileSync(
  join(process.cwd(), "app/api/email-connections/google/connect/route.ts"),
  "utf8",
);
...
const microsoftConnectRoute = readFileSync(
  join(process.cwd(), "app/api/email-connections/microsoft/connect/route.ts"),
  "utf8",
);
...
const oauthState = readFileSync(
  join(process.cwd(), "lib/email/oauth-state.ts"),
  "utf8",
);
...
describe("encrypted email token storage", () => {
  ...
  it("rejects wrong keys, malformed payloads, and missing keys", () => {
    const encrypted = encryptEmailToken("gmail-refresh-token", {
      EMAIL_TOKEN_ENCRYPTION_KEY: "a".repeat(32),
    });

    expect(() =>
      decryptEmailToken(encrypted, {
        EMAIL_TOKEN_ENCRYPTION_KEY: "b".repeat(32),
      }),
    ).toThrow(EmailTokenDecryptionError);
    expect(() =>
      decryptEmailToken("not-a-token", {
        EMAIL_TOKEN_ENCRYPTION_KEY: "a".repeat(32),
      }),
    ).toThrow(EmailTokenDecryptionError);
    expect(() =>
    ...
    expect(() => encryptEmailToken("gmail-token", {})).toThrow(
      EmailTokenEncryptionKeyError,
    );
    expect(() =>
      encryptEmailToken(
    ...
    ).toThrow("Cannot encrypt an empty email token.");
    expect(
      canUseEmailTokenEncryptionKey({ EMAIL_TOKEN_ENCRYPTION_KEY: "short" }),
    ).toBe(false);
  });
  ...
  it("rejects tampered encrypted email token payload components", () => {
    ...
    expect(() => decryptEmailToken(tamperTokenPart(encrypted, 1), env)).toThrow(
      EmailTokenDecryptionError,
    );
    expect(() => decryptEmailToken(tamperTokenPart(encrypted, 2), env)).toThrow(
      EmailTokenDecryptionError,
    );
    expect(() => decryptEmailToken(tamperTokenPart(encrypted, 3), env)).toThrow(
      EmailTokenDecryptionError,
    );
    expect(() =>
      decryptEmailToken(encrypted.replace(/^v1\./, "v2."), env),
    ).toThrow(EmailTokenDecryptionError);
  });
  ...
  it("connects Gmail only through configured OAuth and encrypted persistence", () => {
    ...
    expect(service).toMatch(
      /normalizeProviderAccountEmail\(\s*profile\.email,\s*"Gmail",?\s*\)/,
    );
    expect(service).toMatch(
      /normalizeAccessTokenExpiresAt\(\s*tokenResponse\.expires_in,?\s*\)/,
    );
    expect(service).toMatch(/function normalizeScopes\(\s*scope: unknown,/);
    expect(service).toMatch(/return normalizeScopes\(\s*scope,\s*\[\],?\s*\)/);
    expect(service).toContain("https://oauth2.googleapis.com/tokeninfo");
    expect(service).toContain("EMAIL_OAUTH_GMAIL_SCOPES_MISSING");
    expect(service).toContain(
      "encryptedAccessToken: encryptEmailToken(tokenResponse.access_token",
    );
    expect(service).toContain("encryptedRefreshToken");
    expect(service).toMatch(
      /actionLabel:\s*connection\?\.status === "CONNECTED"\s*\?\s*"Reconnect Gmail"\s*:\s*"Connect Gmail"/,
    );
  ...
  it("adds a safe Gmail full-message diagnostic path without exposing provider secrets", () => {
    ...
    expect(service).toMatch(
      /findGmailConnectionForDiagnostic\(\s*actor\.workspaceId,\s*options\.connectionRef,?\s*\)/,
    );
    expect(service).toMatch(
      /resolveUsableGoogleAccessToken\(\{\s*config,\s*connection,\s*env,\s*fetchImpl,?\s*\}\)/,
    );
    ...
    expect(packageJson).toContain(
      '"gmail:diagnose": "tsx scripts/gmail-diagnose.ts"',
    );
  ...
  it("adds safe Microsoft connect and callback routes without mailbox sync", () => {
    ...
    expect(service).toMatch(
      /normalizeProviderAccountEmail\(\s*profile\.mail \?\? profile\.userPrincipalName,\s*"Microsoft",?\s*\)/,
    );
    expect(service).toContain(
      "encryptedAccessToken: encryptEmailToken(tokenResponse.access_token",
    );
    ...
    expect(microsoftCallbackRoute).toContain(
      'state.provider !== "MICROSOFT_365"',
    );
    expect(microsoftCallbackRoute).toContain(
      "exchangeMicrosoftAuthorizationCode",
    );
  ...
  it("returns stable redacted OAuth errors when providers return non-JSON success responses", async () => {
    const googleConfig = {
      ...
      redirectUri:
        "https://crm.example.test/api/email-connections/google/callback",
    };
    const microsoftConfig = {
      clientId: "microsoft-client",
      clientSecret: "microsoft-secret",
      redirectUri:
        "https://crm.example.test/api/email-connections/microsoft/callback",
    };
  ...
  it("rejects tampered, expired, and unsigned email OAuth states", () => {
    ...
    const tamperedPayload = Buffer.from(
      JSON.stringify({ ...payload, workspaceId: "workspace_other" }),
      "utf8",
    ).toString("base64url");

    expect(() =>
      verifyEmailOAuthState(`${tamperedPayload}.${signature}`, env, now),
    ).toThrow("Email connection state is invalid.");
    expect(() =>
      verifyEmailOAuthState(
        `${encodedPayload}.${tamperBase64Url(signature)}`,
        env,
        now,
      ),
    ).toThrow("Email connection state is invalid.");
    expect(() =>
      verifyEmailOAuthState(`${encodedPayload}.${signature}.extra`, env, now),
    ).toThrow("Email connection state is invalid.");
    expect(() => verifyEmailOAuthState(null, env, now)).toThrow(
      "Email connection state is required.",
    );
    expect(() =>
      verifyEmailOAuthState(
        createEmailOAuthState(
...
function signedState(
  payload: unknown,
  env: { EMAIL_TOKEN_ENCRYPTION_KEY: string },
) {
  const encodedPayload = Buffer.from(JSON.stringify(payload), "utf8").toString(
    "base64url",
  );
  const signature = createHmac("sha256", env.EMAIL_TOKEN_ENCRYPTION_KEY)
